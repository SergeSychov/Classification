...
| 4. Спорные случаи | Judge (OpenRouter) | Арбитраж конфликтов |
...
import / normalize
    ...
    → judge (OpenRouter API — отдельная модель для спорных кейсов)
...
| Judge | **OpenRouter** (другая модель) | OpenRouter API | Спорные ситуации, конфликты, проверки |

**Принципы:**

- DeepSeek — дешёвая модель для массовых раундов (primary, 2A, 2B).
- OpenRouter — более сильная/другая модель только для judge и спорных проверок.
...
5. **Judge** — OpenRouter, арбитраж конфликтов
