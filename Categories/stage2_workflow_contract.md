...
| **Judge —** | Арбитраж спорных случаев (OpenRouter) | **да** |
...
2B — Post-process
  → Judge — Route [judge]
      ...
      → Judge — LLM Prepare → Judge — AI Agent ← Shared — OpenRouter
...
Judge **не** использует DeepSeek — только OpenRouter (см. ниже).

### OpenRouter (Judge)

| Нода | Подключена к | Модель | Credential |
|------|--------------|--------|------------|
| **Shared — OpenRouter** | `Judge — AI Agent` | `openai/gpt-4.1-mini` | OpenRouter account |
...
| Shared — OpenRouter | Chat Model | Модель для Judge |
