#!/usr/bin/env python3
"""Orchestrate Wave-500 MNN v3 Sem (allowlist-wide Load) + mandatory rollback."""
...
def main() -> int:
    ...
    ap.add_argument(
        "--ids-file",
        default=str(ART / "sem_wave500_mnn_v3_allowlist.json"),
    )
    ap.add_argument("--seed", default="sem_wave500_mnn_v3_2026-08-13")
    ap.add_argument("--chunk-size", type=int, default=10)
    ap.add_argument("--out", default=str(ART / "sem_wave500_mnn_v3_report.csv"))
    ap.add_argument(
        "--progress-artifact",
        default=str(ART / "sem_wave500_mnn_v3_progress.json"),
    )
    ap.add_argument("--resume", action="store_true")
    ap.add_argument("--skip-apply", action="store_true")
    args = ap.parse_args()

    n = len(json.loads(Path(args.ids_file).read_text(encoding="utf-8")).get("product_ids") or [])
    marker = {
        "wave": "mnn_v3",
        ...
        "seed": args.seed,
        "n": n,
        "include_classified_load": True,
        "rollback_before_mnn": True,
    }
    marker_path = ART / "sem_wave500_mnn_v3_orchestrator.json"
    ...
    if rc != 0:
        print(f"Sem wave v3 failed rc={rc}; rollback already applied", file=sys.stderr)
        return rc
    print("Sem wave v3 OK; hierarchy-dev rolled back; ready for offline MNN phase")
    return 0
